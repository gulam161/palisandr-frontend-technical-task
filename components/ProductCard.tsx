  ...
  return (
    <div className="group relative bg-white/40 dark:bg-zinc-900 border border-primary/5 hover:border-primary/20 transition-all duration-700 hover:shadow-2xl overflow-hidden p-4 rounded-xl">
      <Link href={`/products/${product.id}`} className="block overflow-hidden relative aspect-4/5 bg-white rounded-lg p-6">
        ...
        <div className="absolute inset-x-0 bottom-4 md:inset-0 md:bottom-auto md:bg-primary/5 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 flex items-end md:items-center justify-center gap-3 md:gap-4 z-10 px-4 pointer-events-none md:pointer-events-auto">
          <button 
            onClick={toggleWishlist}
            className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center transition-all duration-300 transform md:translate-y-10 md:group-hover:translate-y-0 shadow-lg pointer-events-auto ${inWishlist ? "bg-primary text-white" : "bg-white text-primary hover:bg-primary hover:text-white"}`}
          >
          ...
          <button 
            ...
            className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white text-primary hover:bg-primary hover:text-white flex items-center justify-center transition-all duration-300 transform md:translate-y-10 md:group-hover:translate-y-0 delay-100 shadow-lg pointer-events-auto"
          >
            <Icon icon="ion:bag-handle-outline" className="text-xl md:text-2xl" />
          </button>
        </div>
      </Link>
      ...
      <div className="p-4 space-y-4 text-center">
        ...
        <div className="flex flex-col items-center gap-1">
          <div className="flex items-center gap-3">
            <span className="text-primary font-bold font-mono text-lg">₹{(product.price * 80).toLocaleString()}</span>
            <span className="text-primary/40 font-mono text-sm line-through decoration-primary/40">₹{(product.price * 100).toLocaleString()}</span>
          </div>
